...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // --- Permission check for emmely_ai ---
    try {
      ...
      if (bitrixUserId) {
        const { data: permRows } = await supabase
          ...
          .eq("module", "emmely_ai");
        // If there are permission rows configured, check if user is in the list
        if (permRows && permRows.length > 0) {
          const hasAccess = permRows.some((r: any) =>
            // check by querying with user filter
            false
          );
          ...
          if (!userPerm) {
            ...
            </style></head><body><div class="box"><h2>🔒 Sem Permissão</h2><p>Não tem acesso ao módulo Emmely AI.<br>Contacte o administrador.</p></div></body></html>`, { headers: htmlHeaders });
          }
        }
      }
    } catch (e) {
      console.error("[CRM-TAB] Permission check error:", e);
