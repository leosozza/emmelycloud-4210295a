...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const format = url.searchParams.get("format"); // "html" returns rendered page
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .badge {
      display: inline-block; margin-top: 16px; padding: 6px 20px; border-radius: 20px;
      background: rgba(16, 185, 129, 0.2); color: #10b981; font-size: 12px;
      font-weight: 600; letter-spacing: 1px; border: 1px solid rgba(16,185,129,0.3);
    }
    ...
    .section-title {
      font-size: 10px; text-transform: uppercase; letter-spacing: 2px; color: #64748b;
      font-weight: 600; margin-bottom: 10px; border-bottom: 1px solid #e2e8f0; padding-bottom: 6px;
    }
    ...
    .hash-box {
      background: #f1f5f9; border-radius: 8px; padding: 16px; margin-top: 8px;
      font-family: 'Courier New', monospace; font-size: 12px; word-break: break-all;
      border: 1px solid #e2e8f0; color: #334155; line-height: 1.6;
    }
    .legal {
      margin-top: 30px; padding: 20px; background: #fffbeb; border-radius: 8px;
      border: 1px solid #fde68a; font-size: 11px; color: #92400e; line-height: 1.7;
    }
    .legal strong { color: #78350f; }
    .footer {
      text-align: center; color: #94a3b8; font-size: 10px; padding: 20px 40px;
    ...
    .seal {
      ...
      border: 3px solid #10b981; text-align: center; line-height: 54px;
      font-size: 10px; font-weight: 700; color: #10b981; letter-spacing: 1px;
      margin: 0 auto 12px; background: rgba(16,185,129,0.05);
    }
  </style>
</head>
<body>
  ...
  <div class="content">
    ...
    <div class="section">
      ...
      <table class="info">
        ...
        <tr><td class="label">ID do Contrato</td><td><code style="font-size:11px;color:#64748b;">${contract.id}</code></td></tr>
      </table>
    </div>` : ""}
    ...
    <div class="section">
      ...
      <p style="font-size:11px; color:#94a3b8; margin-top:6px;">
        Este hash criptográfico garante a integridade e autenticidade da assinatura. Qualquer alteração nos dados invalidaria este hash.
      </p>
    </div>
    ...
    <div class="legal">
      ...
      Este certificado atesta que o documento acima identificado foi assinado digitalmente pelo signatário mencionado, 
      utilizando o método indicado, com registo de todas as evidências necessárias para a sua validade jurídica.<br><br>
  ...
  <div class="footer">
    ...
    // If format=html, return the HTML directly for rendering
    if (format === "html") {
    ...
    // Store the certificate HTML
    const fileName = `certificate-${contract.id}.html`;
